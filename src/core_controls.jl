...
const _SCALES = Dict{Symbol,Vector{Int}}(
    ...
    :natural_minor    => [0, 2, 3, 5, 7, 8, 10],   # = minor / aeolian
    ...
    :melodic_major    => [0, 2, 4, 5, 7, 8, 10],   # aka hindu
    :locrian_major    => [0, 2, 4, 5, 6, 8, 10],
    :super_locrian    => [0, 1, 3, 4, 6, 8, 10],   # altered scale

    # Pentatonic / 5-note
    :pentatonic       => [0, 2, 4, 7, 9],          # = major pent (legacy)
    ...
    # Indian ragas (a sampling — full list is huge)
